...
const StyledRow: StyledComponent<any, any> = styled.tr`
  cursor: ${({ clickable }: TArchiveRow) => clickable ? "pointer" : "default"};
  ...
  & > td {
    ...
    &:nth-of-type(n + 3) {
      filter: ${({ encrypted }: TArchiveRow) => encrypted ? `blur(${px2rem(6)})` : "none"};
    }
    ...
    &:nth-of-type(7) {
      ...
      & > button {
        display: ${({ encrypted }: TArchiveRow) => encrypted ? "block" : "none"};
        position: absolute;
        top: ${px2rem(20)};
        right: 25vw;
        width: ${px2rem(80)};
...
`;
const ArchiveRow: FunctionComponent<TArchiveRow> = ({
  checked,
  avatarColor,
  name,
  subject,
