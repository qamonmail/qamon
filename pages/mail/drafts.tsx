...
const Drafts: NextPage<TPageProps> = ({ title }) => {
      ...
      {isLg && (
        <DashboardLayout>
          ...
          <ContentHeader title={title?.split("-")?.[0]?.trim()} withLock={true} onLockToggle={handleLockToggle}>
            <FormattedMessage
              id="your-drafts-mail"
              defaultMessage="Your draft mails are saved in your browser so you won't access on other device or if you clean your cache."
              tagName="span" />
            <SearchForm value={""} />
          </ContentHeader>
          {pending && (
